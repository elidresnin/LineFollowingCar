...
#define angle_centre 88
byte angle_difference_xs = 4;
byte angle_difference_small = 8;
byte angle_difference_midsmall = 12;
byte angle_difference_medium = 16;
byte angle_difference_midlarge = 20;
byte angle_difference_large = 24;
byte angle_difference_xl = 28;
byte angle_difference_xxl = 30;
byte angle_left_xs;
...
byte motor_stop = 0;
byte motor_speed_fast = 226;
byte motor_speed_slow = 226;
#define ena_pin 5
#define enb_pin 6
...
// int countersteering_duration = 100; // Adjust this value to control the duration of countersteering (in milliseconds)
// int countersteering_angle = 30; // Adjust this value to set the steering angle during countersteering
// int countersteeringTimer = 0;
...
void setup() {
  ...
  angle_left_xs = angle_centre - angle_difference_xs;
  angle_left_small = angle_centre - angle_difference_small;
  angle_left_midsmall = angle_centre - angle_difference_midsmall;
  angle_left_medium = angle_centre - angle_difference_medium;
  angle_left_midlarge = angle_centre - angle_difference_midlarge;
  angle_left_large = angle_centre - angle_difference_large;
  angle_left_xl = angle_centre - angle_difference_xl;
  angle_left_xxl = angle_centre - angle_difference_xxl;

  angle_right_xs = angle_centre + angle_difference_xs;
  angle_right_small = angle_centre + angle_difference_small;
  angle_right_midsmall = angle_centre + angle_difference_midsmall;
  angle_right_medium = angle_centre + angle_difference_medium;
  angle_right_midlarge = angle_centre + angle_difference_midlarge;
  angle_right_large = angle_centre + angle_difference_large;
  angle_right_xl = angle_centre + angle_difference_xl;
  angle_right_xxl = angle_centre + angle_difference_xxl;
}
...
void loop() {
  ...
  //****************************************************************
  {
    ...
    else if ((ir_data[2] < threshold) && (ir_data[0] < threshold))
      tangential_position = 2;
    else if (ir_data[2] < threshold)
      tangential_position = 3;
    else if ((ir_data[2] < threshold) && (ir_data[4] < threshold))
      tangential_position = 4;
    else if (ir_data[4] < threshold)
      tangential_position = 5;
    else if ((ir_data[4] < threshold) && (ir_data[6] < threshold))
      tangential_position = 6;
    else if (ir_data[6] < threshold)
      tangential_position = 7;
    else if ((ir_data[6] < threshold) && (ir_data[8] < threshold))
      tangential_position = 8;
    else if (ir_data[8] < threshold)
      tangential_position = 9;
    else if ((ir_data[8] < threshold) && (ir_data[10] < threshold))
      tangential_position = 10;
    else if (ir_data[10] < threshold)
      tangential_position = 11;
    else if ((ir_data[10] < threshold) && (ir_data[12] < threshold))
      tangential_position = 12;
    else if (ir_data[12] < threshold)
      tangential_position = 13;
    else if ((ir_data[12] < threshold) && (ir_data[14] < threshold))
      tangential_position = 14;
    else if (ir_data[14] < threshold)
      tangential_position = 15;
    else
      flag = 2;
  }
  //****************************************************************
  ...
  // Determine servo motor angle based on tangential position
{
  if (countersteering) // If countersteering is active, steer in the opposite direction
  {
    if (tangential_position == 1)
      angle = angle_right_xl;
    else if (tangential_position == 2)
      angle = angle_right_large;
    else if (tangential_position == 3)
      angle = angle_right_midlarge;
    else if (tangential_position == 4)
      angle = angle_right_medium;
    else if (tangential_position == 5)
      angle = angle_right_midsmall;
    else if (tangential_position == 6)
      angle = angle_right_small;
    else if (tangential_position == 7)
      angle = angle_right_xs;
    else if (tangential_position == 8)
      angle = angle_centre;
    else if (tangential_position == 9)
      angle = angle_left_xs;
    else if (tangential_position == 10)
      angle = angle_left_small;
    else if (tangential_position == 11)
      angle = angle_left_midsmall;
    else if (tangential_position == 12)
      angle = angle_left_medium;
    else if (tangential_position == 13)
      angle = angle_left_midlarge;
    else if (tangential_position == 14)
      angle = angle_left_large;
    else if (tangential_position == 15)
      angle = angle_left_xl;
    if (flag == 1)
      angle = angle_centre;
  }
  else // If not countersteering, continue with normal steering behavior
  {
    if (tangential_position == 1)
      angle = angle_left_xl;
    else if (tangential_position == 2)
  ...
  if (flag == 2)
  {
    ...
    // Steer in the opposite direction for countersteering
    if (tangential_position < 8)
      angle = (angle_centre + countersteering_angle);
    else
      angle = (angle_centre - countersteering_angle);
      
    countersteeringTimer = countersteering_duration;
  }

  // Decrease the countersteeringTimer if it's active
  if (countersteeringTimer > 0)
  {
    countersteeringTimer--;
    if (countersteeringTimer == 0)
    {
      // Countersteering duration has elapsed, reset the countersteering flag and return to normal steering
